 ...
 * Use this in `test.beforeAll` to skip tests that require the emulator:
 *
 * ```ts
 * test.beforeAll(async () => {
 *   await requireEmulator(test);
 * });
 * ```
 */
export async function checkEmulatorAvailable(): Promise<boolean> {
  try {
    ...
    const res = await fetch('http://127.0.0.1:9099', { signal: controller.signal }).catch(() => null);
    clearTimeout(timer);
    return res !== null;
  } catch {
    return false;
