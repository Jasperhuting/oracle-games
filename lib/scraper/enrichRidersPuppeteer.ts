...
export async function enrichRidersPuppeteer({ year, team }: { year: number, team: string }) {
    const url = `https://www.procyclingstats.com/team/${team}`;

    console.log('Enriching riders with Puppeteer:', url);
    ...
    try {
        ...
        riderElements.each((_, el) => {
            ...
            const rider: EnrichedRider = {
                ...
                age: '', // Will be filled in later from the age table
            };
            if (riderName) {
                riders.push(rider);
            }
        ...
        console.log(`Successfully enriched team "${team}" with ${riders.length} riders`);

        return result;
    } catch (error) {
        console.error(`\n❌ ERROR in enrichRidersPuppeteer for team "${team}":`, error);
        throw error;
    } finally {
        await browser.close();
    }
