...
export const POST = adminHandler('create-race', async ({ uid, request }) => {
  const { name, year, slug, description } = await request.json();

  if (!name || !year) {
    throw new ApiError('Name and year are required', 400);
  }
  ...
  if (existingRace.exists) {
    throw new ApiError('A race with this name and year already exists', 409);
  }
  ...
  // Fetch admin doc for activity logs
  ...
  try {
    ...
    throw new ApiError(`Could not load starting list: ${errorMessage}. Race is not created.`, 400);
  }
