...
import { drivers as hardcodedDrivers, races2026 as hardcodedRaces } from '@/app/f1/data';
...
// GET /api/f1/migrate - Check status
...
// POST /api/f1/migrate - Run migration
export async function POST() {
  try {
    ...
    for (const [teamName, teamData] of teamsMap) {
      ...
      if (teamData.colorAlt) teamDoc.colorAlt = teamData.colorAlt;
      if (teamData.carImage) teamDoc.carImage = teamData.carImage;

      teamsBatch.set(docRef, teamDoc);
      results.teams++;
    }
    ...
    for (const driver of hardcodedDrivers) {
      ...
      if (driver.numberImage) driverDoc.numberImage = driver.numberImage;

      driversBatch.set(docRef, driverDoc);
      results.drivers++;
    }
