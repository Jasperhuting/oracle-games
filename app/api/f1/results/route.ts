...
import { calculatePredictionPoints, calculateCorrectPredictions } from '@/app/f1/lib/points';
...
export async function POST(request: NextRequest) {
  try {
    // Check admin - temporarily disabled for testing
    // if (!(await isAdmin())) {
    //   return NextResponse.json(
    ...
    // Support both formats: { result: {...} } or direct { season, round, finishOrder, ... }
    const result = body.result || body;

    if (!result.season || result.round === undefined) {
      return NextResponse.json(
        { success: false, error: 'Missing required result fields (season, round)' },
        { status: 400 }
      );
    }
    ...
    if (!result.finishOrder || result.finishOrder.length !== 22) {
      return NextResponse.json(
        { success: false, error: `Result must have exactly 22 drivers (got ${result.finishOrder?.length || 0})` },
        { status: 400 }
      );
    }
    ...
    // Apply 100-point penalty to active participants who submitted no prediction.
    ...
    for (const update of pointsUpdates) {
      ...
      if (standingDoc.exists) {
        ...
        const previousRacePoints =
          typeof existing.racePoints === 'object' && existing.racePoints !== null
            ? existing.racePoints
            : {};
        ...
        await standingRef.update({
          ...
          racesParticipated: Object.keys(newRacePoints).length,
