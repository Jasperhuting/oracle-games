...
export default function GroupsPage() {
    ...
    const [poules, setPoules] = useState<PouleRanking[]>([]); // For auto-calculated standings (immutable)
    const [manualRankings, setManualRankings] = useState<PouleRanking[]>([]); // For manual predictions
    ...
    const fetchPoulesAndMatches = async () => {
        try {
            ...
            // Initialize manual rankings as a copy of the loaded poules
    ...
    const handleDropToPosition = (position: number) => {
        ...
        if (teamAtPosition && teamAtPosition.id !== draggedTeam.id) {
            // Swap positions - update both team positions
    ...
    return (
        <div className="p-6">
            ...
            <div className="mb-8">
                <h2 className="text-xl font-semibold mb-4">Manual Ranking - Poule {selectedPoule.toUpperCase()}</h2>
                <p className="text-sm text-gray-600 mb-4">Drag teams to set your predicted ranking</p>
            ...
            <div className="mb-8">
                <h2 className="text-xl font-semibold mb-4">Auto-Calculated Standings - Poule {selectedPoule.toUpperCase()}</h2>
                <p className="text-sm text-gray-600 mb-4">Automatically calculated from match results</p>
            ...
            {/* Matches Section */}
            <div className="mb-8">
                <h2 className="text-xl font-semibold mb-4">Matches - Poule {selectedPoule.toUpperCase()}</h2>
                    ...
                    onClick={async () => {
                        try {
                            ...
                            manualRankings.forEach(poule => {
                                poule.rankings.forEach(team => {
                                    if (team) {
                                        teamsWithRankings.push(team);
                                    }
                                });
                            });

                            const rankingsResponse = await fetch('/api/wk-2026/updatePot', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ teams: teamsWithRankings })
                            });

                            // Save match scores
                            const matchesResponse = await fetch('/api/wk-2026/saveMatches', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ matches })
                            });

                            if (rankingsResponse.ok && matchesResponse.ok) {
                                alert('Rankings and scores saved successfully!');
                            } else {
